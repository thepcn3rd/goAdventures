...
func main() {
	...
	if !(cf.FileExists("/config.json")) {
		...
		b64configDefault := ""
	...
	for optionSelected != "e" && optionSelected != "E" && optionSelected != "exit" {
		...
		if strings.Contains(optionSelectedLower, "show help") {
			...
			if config.ReqOutputSave == "true" {
				...
				if config.RunWebServer == "true" {
					outputPath = "static/" + config.ReqPath
				} else {
					outputPath = config.ReqPath
				}
				f, err := os.Create(outputPath)
				cf.CheckError("Unable create file to save output", err, true)
				defer f.Close()
				fmt.Printf(colorGreen+"\nSaved the file to %s\n\n"+colorReset, config.ReqPath)
				f.Write(respBody)
				f.Close()
			}
		}
