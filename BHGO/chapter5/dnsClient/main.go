...
// GOOS=linux GOARCH=amd64 go build -o dnsClient.bin -ldflags "-w -s" main.go
...
// GOOS=linux GOARCH=amd64 CGO_ENABLED=0 go build -o dnsClient.bin main.go

// To cross compile windows
// GOOS=windows GOARCH=amd64 go build -o dnsClient.exe -ldflags "-w -s" main.go
...
func main() {
	...
	// The following DNS servers I have used...
	...
	if *domainPtr != "" {
		...
		if strings.Contains(*ipPTR, "/") {
			...
			for ip := range ipAddressesChan {
				...
				// Rate limiting the CIDR option for 2 seconds between queries
