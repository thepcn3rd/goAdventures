...
{
	"port": "443",
	"sourceURL": "example.proxy.local",
	"destURL": "www.original.domain"
}
...
func main() {
	...
	proxy := http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		if strings.Contains(req.Host, config.SourceURL) {
			dstURL = "https://" + config.DestURL + req.RequestURI
		} else {
		...
		for key, values := range req.Header {
			for _, value := range values {
				...
				if key == "Referer" && strings.Contains(value, config.SourceURL) {
					//Note this is not URL Encoded
					value = strings.Replace(value, "https://"+config.SourceURL, "https://"+config.DestURL, -1)
					fmt.Printf("\n*** Modified the Referer header to: %s\n\n", value)
					dstServerReq.Header.Add(key, value)
				} else if key == "Cookie" && strings.Contains(value, config.SourceURL) {
					// Note the values of the cookie are URL Encoded
					fmt.Println("\n*** Evaluating the cookies and modifying them...")
					value = strings.Replace(value, "https%3A%2F%2F"+config.SourceURL, "https%3A%2F%2F"+config.DestURL, -1)
		...
		bodyString = strings.Replace(bodyString, config.DestURL, config.SourceURL, -1)
